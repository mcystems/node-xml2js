...
describe('parser tests', () => {
  ...
  it('test parse with explicitChildren', async () => {
    ...
    expect(oc(r).sample.$$().find(v => v.name === 'chartest').$.desc).to.equals('Test for CHARs');
    ...
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[0]._)
      ...
      .equals(
        '\n            This  is\n            \n            character\n            \n            data!\n            \n        ');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[0].$$[0]._).to.equals('Foo(1)');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[0].$$[1]._).to.equals('Foo(2)');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[0].$$[2]._).to.equals('Foo(3)');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[0].$$[3]._).to.equals('Foo(4)');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[1]._).to.equals('Qux.');
    expect(oc(r).sample.$$().find(v => v.name === 'listtest').$$[2]._).to.equals('Quux.');
    expect(oc(r).sample.$$().find(v => v.name === 'nochildrentest').$$).to.equals(undefined);
  });
  ...
  it('test empty tag result specified null', async () => {
    const r = await skeleton({...parserDefaults, emptyTag: undefined});
  ...
  it('test child node without explicitArray', async () => {
    ...
    expect(oc(r).sample.arraytest.item[0].subitem._()).to.equals('Baz.');
    expect(oc(r).sample.arraytest.item[1].subitem[0]._()).to.equals('Foo.');
    expect(oc(r).sample.arraytest.item[1].subitem[1]._()).to.equals('Bar.');
  });

  it('test child node with explicitArray', async () => {
    const r = await skeleton({...parserDefaults, explicitArray: true});

    expect(oc(r).sample.arraytest[0].item[0].subitem[0]._()).to.equals('Baz.');
    expect(oc(r).sample.arraytest[0].item[1].subitem[0]._()).to.equals('Foo.');
    expect(oc(r).sample.arraytest[0].item[1].subitem[1]._()).to.equals('Bar.');
  });
  ...
  it('test validator', async () => {
    ...
    expect(typeof oc(r).sample.validatortest[0].stringtest[0]._()).to.equals('string');
    ...
    expect(oc(r).sample.validatortest[0].oneitemarray[0].item[0]._()).to.equals('Bar.');
  ...
  it('test CDATA whitespaces result', async () => {
    ...
    const r = await parseString(xml);
    expect(oc(r).spacecdatatest._()).to.equals(' ');
  });
  ...
  it('test cdata-named node', async () => {
    ...
    const r = await parseString(xml);
    expect(oc(r).test.cdata[0]._()).to.equals('hello');
  });
  ...
  it('test single valueProcessor', async () => {
    ...
    expect(oc(r).sample.valueProcessTest[0]._()).to.equals('SOME VALUE');
  });

  it('test multiple valueProcessor', async () => {
    const r = await skeleton({...parserDefaults, valueProcessors: [nameToUpperCase, nameCutoff]});
    expect(oc(r).sample.valueProcessTest[0]._()).to.equals('SOME');
  });

  it('test single tagNameProcessors', async () => {
    const r = await skeleton({...parserDefaults, tagNameProcessors: [nameToUpperCase]});
    expect(oc(r).SAMPLE().name).to.equals('SAMPLE');
    expect(oc(r).SAMPLE.TAGNAMEPROCESSTEST[0]().name).to.equals('TAGNAMEPROCESSTEST');
  });
