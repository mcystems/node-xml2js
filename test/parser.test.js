...
module.exports = {
  ...
  'test async execution'(test) {
    return fs.readFile(fileName, (err, data) =>
      xml2js.parseString(data, {async: true}, function(err, r) {
        equ(err, null);
        equ(r.sample.chartest[0]._, 'Character data here!');
        return test.finish();
      })
  ...
  'test single valueProcessor': skeleton({valueProcessors: [nameToUpperCase]}, function(r){
    console.log(`Result object: ${util.inspect(r, false, 10)}`);
    return equ(r.sample.valueProcessTest[0], 'SOME VALUE');
  }),

  'test multiple valueProcessor': skeleton({valueProcessors: [nameToUpperCase, nameCutoff]}, function(r){
    console.log(`Result object: ${util.inspect(r, false, 10)}`);
  ...
  'test single tagNameProcessors in simple callback'(test) {
    return fs.readFile(fileName, (err, data) =>
      xml2js.parseString(data, {tagNameProcessors: [nameToUpperCase]}, function(err, r){
        console.log(`Result object: ${util.inspect(r, false, 10)}`);
        equ(r.hasOwnProperty('SAMPLE'), true);
